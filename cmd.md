...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.8.2.4 异常流程：密码太短
我们不希望用户选择一个弱密码---这会把他们的账户暴漏给攻击者；
OWASP提供了一个套关于密码强度的最低要求---密码的长度应该大于12个字符，但小于128个字符；
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.8.2.5 退出登录
退出状态是一个改变状态的操作：需要通过HTML按钮使用POST方法；
我们使用的是基于会话的身份验证---如果有一个与会话状态中的user_id键相关的有效的用户ID,那么用户就是"登录"了，要是退出登录，只需要删除会话---从存储后端删除状态，并取消客户端Cookie设置；
actix-sesion用Session::purge方法来实现这个功能，需要把它暴漏在TypedSession的抽象中；
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.8.2.6 正常流程：密码修改成功
企业应用程序通常需要十几个步骤来执行真实的业务流程；
(原书上代码有问题)
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.9 重构
出于对速度的考虑，我们在多个请求处理函数中复制了相同的认证逻辑---看看是否有更好的解决方案

方式一:提取认证逻辑，重构一个函数；其中需要注意的是：使用actix_web::error::InternalError::from_response()方法将重定向响应移动到错误路劲上，以便在处理函数中使用?操作符；
方式二:可以尝试实现一个中间件来处理这个问题；
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.9.1 如何实现一个actix-web中间件
actix_web_lab crate 用于测试actix-web框架的实验特性，采用更快地发布策略；
当使用中间件并从传入请求中提取信息时，可以通过扩展请求来解决这个问题；中间件将它想传递给下游请求处理函数的信息，添加到传入请求的类型映射中(request.extensions_mut()),然后
请求处理函数可以使用ReqData提取器访问它；

在App上调用wrap(),会把中间件应用于所有的路由；考虑到目标端点共享相同的基础路径，可以通过引入一个web::scope()只将中间件逻辑应用于 /admin/* 端点,即
通过在web::scope("admin")而不是App上调用wrap,将中间件仅限于 /admin/* ;
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
10.10 为Post /admin/newsletters添加基于会话的身份验证
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11 容错的工作流
通过Postmark向所有订阅者发送电子邮件，每次只调用一次API,如果订阅者较少，那么这样做已经足够好了，但如果要处理数百个订阅者，则会出现各种问题，因此我们希望应用程序具有兼容性：对邮件简报的发送，
不应因为应用程序崩溃、Postmark API错误或者网络超时等短暂故障而中断。为了在故障时提供可靠的服务，我们必须学习新的概念:幂等、加锁、队列和后台任务；
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.1 重温 POST /admin/newsletters
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.2 我们的目标
尽最大努力送达：向所有订阅者发送一期邮件简报；
不能保证所有邮件都能送达：有些账号可能刚刚被删除；
同时，尽量减少重复发送，避免订阅者重复收到同一份邮件；
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3 失败模式：看看POST /admin/newsletters端点可能出现的失败模式
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.1 无效输入：传入的请求可能存在问题
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.2 网络I/O: 通过网络与其它机器交互时，可能出现问题
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.2.1 Postgres:尝试检索当前的订阅者列表时，数据可能出现问题
出了重试，没有其他选择：
    - 在进程内重试：在get_confirmed_subscribers调用的周围添加一些逻辑；
        - 该方式是应用程序更能抵御临时故障，但重试次数有限，最终还是放弃；
    - 放弃重试：向用户返回错误信息，然后，用户可以决定是否重试；
        - 若一开始就选择该方式，可能导致400报错，但与总体目标并不冲突；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.2.2 Postmark --- API错误
第一种：当前实现方式是:若出现错误，会终止处理并向调用者返回500 InternalServerError,而我们是按照顺序依次发送邮件，一遇到API错误就终止，将永远没有机会向其他订阅者发送新的邮件简报，这远不是尽最大努力送达；
第二种：能否重试提交表单？重试或不重试都有问题；
以上，我们会意识到这种矛盾：我们正在处理一个由多个任务组成的工作流；
参考使用Postgres数据库时使用了事务的全有或者全无的特性：除非所有操作都成功，否则什么都不会发生；
Postmark每个接口调用都是独立的工作单元，不提供任何事务语义，没办法将他们连接在一起；
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.3 应用程序崩溃
应用程序可能随时崩溃，特别的是，崩溃可能发生在开始处理订阅者列表之后、处理完之前；重新提交表单，很可能会导致大量多余的投递；
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.3.4 作者的操作
可能会遇到作者与API之间的交互问题：若服务器响应时间长，作者可能会不耐烦，重新提交表单，浏览器可能会决定放弃(客户端超时)，或者多次点击“提交”按钮；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.4 幂等简介
重试安全 对应用程序接口的人性化有着巨大的影响；
在当前领域中，对重试的含义，直观的理解是：给每个订阅者发送邮件简报的次数不得超过一次；但，如果换一个领域呢？业界并没有一个公认的明确定义；
本书中，对重试安全定义如下：
    - 如果调用者**无法观察到请求是否已被发送到服务器一次或多次**，那么API端点就是重试安全的(或幂等的)；
另一个用来描述重试安全概念的术语是：幂等，它们是同义词；幂等，是行业术语；
幂等的定义是围绕可观测性概念而建立的，即调用者可以通过与系统本身的交互来检查系统状态；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.4.1 幂等实践：支付
本节探讨在支付领域中幂等的定义。虚构的支付API提供三个端点：
    - GET /balance，检查当前的账户余额；
    - GET /payments, 检查支付列表；
    - POST /payments, 发起新支付；
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.4.2 幂等键
鉴于一切都是为了理解转账人的意图，没有比授权转账人告诉我们，他们想要做什么更好的策略了。这通常是通过 幂等键 来实现；
转账人为要执行的每一个改变状态的操作生成一个唯一标识符，即 幂等键；在传出的请求中加上 幂等键，通常加在HTTP请求头上(例如:Idempotency-key);
服务器现在可以很容易地发现重复的请求：
    - 两个相同的请求，不同的 幂等键 等于 两个不同的操作；
    - 两个相同的请求，相同的 幂等键 等于 一个操作， 第二个请求时重复的；
    - 两个不同的请求，相同的 幂等键 等于 第一个请求被处理，第二个请求被拒绝；
我们要求在POST /admin/newsletters 中提供 幂等键，作为幂等实现的一部分；
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.4.3 并发请求
同时发起两个重复的请求时，并行处理，可能产生两笔不同的支付，带来执行风险；
通常的做法是引入同步：在第一个请求被处理完成后，再处理第二个请求；我们有两种选择：
    - 拒绝第二个请求，向调用者返回409请求冲突的状态码；
    - 等待，直到第一个请求被处理完成；

考虑用例(处理表单)，将采用第二种策略，以尽量减少用户可见错误的数量---浏览器不会自动重试409;
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.5 测试需求(第一部分)：希望得到一个成功的响应，并且不能重复发送邮件简报；
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.6 实现策略
有两种选择：
    - 一种是需要状态；
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.6.1 有状态的幂等:保存和重放
先处理第一个请求，将其幂等键存储到即将返回的HTTP响应中，当有重试请求时，在存储中查找与幂等键相匹配的请求，获取所保存的HTTP响应并将其返回给调用者。Postmark的API不会被再次调用，从而避免了重复发送；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.6.2 无状态的幂等：确定性的键生成
无状态方法，是试图在不依赖持久化的情况下实现相同的结果；
对于每个订阅者，根据在请求上附上的user_id、邮件内容指纹、幂等键，确定性的生成一个新的幂等键。每次调用Postmark，都会确保 发送订阅者 特定的幂等键。当重试请求时，执行相同的处理逻辑，将导致使用
完全相同的幂等键对Postmark进行同样的HTTP调用，如果正确的实现了幂等，就不会发送新邮件了；

即使同一user_id,两个不同的邮件简报内容不应生成相同的订阅者特定的幂等键；如果出现这种情况，Postmark的幂等逻辑会阻止第二份邮件的发送；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.6.3 时间问题有些棘手
无状态方法和有状态方法并不完全相同；
当采用无状态方法时，初始请求和随后的重试请求之间的时间间隔会影响处理结果；
***原文值得好好一读***
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.6.4 做出选择
Postmark的API不提供任何 幂等 机制，因此不能采用无状态方法；因此学一些新模式；
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.7 幂等存储
------------------------
11.7.1 应该使用那种数据库
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.7.2 数据库模式
(user_id, idempotency_key, http_response)三元组
用户ID和幂等键可被用作联合主键，以及每一行记录的创建时间，以便淘汰幂等键；
Postgres不支持元组数组，但有一种解决办法：可以定义一个Postgres组合类型，即一个命名的字段集合，相当于Rust代码中的结构体：
CREATE TYPE header_pair AS (
    name TEXT,
    value BYTEA
);

本可以定义一个完整的 http_response 组合类型，但会在sqlx中遇到bug，这是由Rust编译器中的一个错误引起，当前最好避免嵌套组合类型；
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8 保存和重放
----------------
11.8.1 读取幂等键
POST /admin/newsletters 端点是由HTML表单提交触发的，因此无法控制发送到服务器的请求头信息；最实际的做法是 在表单数据中 嵌入幂等键；

anyhow::bail!宏， 实际上是一个语法糖，快速失败：立即创建错误并返回，比手动 return Err(...) 更简洁；
anyhow::bail!("Data cannot be empty"),相当于return Err(anyhow::anyhow!("Data cannot be empty"));
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8.2 查询已保存的响应：从数据库中获取已保存的HTTP响应(假设存在)；

sqlx 不知道如何处理自定义的header_pair类型，但它的Type、Decode、Encode trait 机制可以指定如何处理它，不用自己实现，可以用宏来派生他们，
只需指定类型字段和组合类型的名称，就像他在Postgres中展示的那样；但sqlx::query！不会自动处理自定义类型，需要使用显式类型注解来处理自定义的列；

fetch_optional: 查询可能有0或1条结果
fetch_one: 必须返回1条结果，否则报错
fetch_all: 返回所有结果（0-N条）
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8.3 保存响应
将HttpResponse分解成不同的部分：.status()获取状态码，.header()获取响应头，.body()获取响应体；
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8.3.1 MessageBody和HTTP流
响应一直是在服务器上处理完成后再返回给调用者的。HTTP/1.1支持另一种数据传输机制---Transfer-Encoding:chunked,也被称为"HTTP流"，
服务器将请求负载拆分为多个块，然后一次一个的发送给调用者，而不是先在内存中累计整个请求体。这样服务器可以大大减少内存使用量。在处理大型请求负载(如文件或大型查询结果)时，
这种方法非常有用(全程流式传输)；
提到HTTP流，就更容易理解MessageBody的设计了；在actix-web中使用类型作为body时，必须实现MessageBody Trait:可以一次获取一个块数据，直到全部获取完毕；

当响应不是流式传输时，数据可以被一次性获取到---poll_next可以一次性返回所有数据；

BoxBody是HttpResponse使用的默认body类型；BoxBody抽象了特定的请求负载的传输机制。在底层，他只不过是一个枚举，每种策略都有一个变体，其中还有一个特例用于没有响应体的响应；

分块传输编码 允许服务器在不知道内容总长度的情况下逐步发送数据，特别适用于：
    - 动态生成的内容
    - 大文件或实时数据流
    - 需要边生成边传输的场景
***以上内容值得原书重点细读***

流式数据处理中的一个重要特性：从请求负载中提取数据块需要获取对流本身的可变引用--- 一旦读取了数据块，就无法重放流并再次读取它；
流的基本特性：
    - 单向性：数据只能向前读取，不能后退
    - 消耗性：一旦读取，数据就从流中移除
    - 状态性：流内部维护读取位置状态
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8.3.2 Postgres 组合类型的数组
在数据库插入数据的原代码中，我们使用的是自定义类型，而sqlx::query!功能不够强大，无法在编译时了解它以进行检查，必须关闭编译时检查功能，使用query_unchecked!代替query!;

通过#[sqlx(type_name="header_pair")]属性，sqlx知道组合类型本身的名称，但不知道包含header_pair元素的数组类型名称；在运行CRATE TYPE语句时，Postgres会隐式创建一个数组类型：它
就是以下划线类前缀的组合类型名称(即"_header_pair")；

---------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.8.3.3 整合代码

书中的源代码，在数据库保存resposne的同时，但未保存FlashMessage信息，导致后面访问返回的用户可读的信息与首次不一样，故需要将测试代码最后一步改为取反；

---------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.9 并发请求
现在处理更复杂的场景：重试请求在第一个请求被处理完成之前到来；
我们希望第二个请求排在第一个请求之后，一旦处理完成，他就会从数据库中获取所保存的HTTP响应，并将其返回给调用者；

--------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.9.1 测试需求(第二部分)
测试驱动的代码的结果与 原书结果不一致，未解决；
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
11.9.2 同步
第二个请求在尝试插入数据库之前，并不知道第一个请求的存在。如果要防止重复发送，则需要在开始处理订阅者之前引入 "跨请求同步";

如果所有传入的请求都有单个API实例提供服务，那么使用内存锁(如tokio::sync::Mutex)就可以；但我们的情况并非如此：API是多副本的，因此两个请求可能由不同实例处理；
我们的同步机制必须在进程外进行---数据库是最合适的选择；

idempotency表中，用户user_id和幂等键 的 每个唯一组合都有一条记录；

当前的实现是 在处理完请求后向idempotency表中插入一条记录，然后将相应返回调用者。我们将改变为：在处理器被调用后立即插入一条记录；

如果第一个请求已完成，希望返回已保存的响应，可以使用ON CONFLICT语句来处理---它允许我们定义当INSERT因为违反约束(如唯一性)而失败时应该发生的情况，可以通过检查受语句影响的记录数 
来检测 记录是否已经存在；
***原书这段值得细度**

要求sqlx强制假设这些列不会为空，其语法是 必须在列的别名后加上 '!' ;
